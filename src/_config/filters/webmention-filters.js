...
export function webmentionsForUrl(url, webmentions) {
  if (!Array.isArray(webmentions)) {
    console.warn(url + ' - webmentions is not an array');
    return {
      total: 0,
      likes: [],
      reposts: [],
  ...
  const total = webmentions
    ...
    .filter((obj) => obj.author.url !== 'https://bsky.app/profile/jasonbyday.bsky.social')
    .length;
